									...
									<tbody>
										...
										if (!empty($report)) : foreach ($report as $data) : if ($data->report_state != $state) {
												...
												<tr>
													...
													$Date = $data->sign_off_date;
													$end_date = date('d-m-Y', strtotime(trim($Date). ' + 60 days'));
													$my_date = date('d-m-Y');

													$from=date_create($my_date);
													$to=date_create($end_date);

													$diff=date_diff($from,$to);
													$daysleft = str_replace("+"," ",$diff->format('%R%a'));

													?>
													<?php if ($daysleft <= 0) {
													echo '<td><span class="label bg-red">'.$daysleft.'</span></td>';
													} else {
													echo '<td><span class="label bg-green">'.$daysleft.'</span></td>';
													}
													?>
												</tr>
										<?php endforeach;
