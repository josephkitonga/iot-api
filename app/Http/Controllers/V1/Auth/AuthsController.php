...
class AuthsController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            DB::table('password_resets')->insert(['email' => $user->email, 'token' => $activation_code, 'created_at' => Carbon::now()->toDateTimeString()]);
            Mail::to($email)->send(new RegisterUser($email,$activation_code,$name));
    ...
    public function invite(Request $request)
    {
        ...
        try {
            ...
            DB::table('password_resets')->insert(['email' => $email, 'token' => $activation_code, 'created_at' => Carbon::now()->toDateTimeString()]);
            Mail::to($email)->send(new RegisterUser($email,$activation_code,$name));

            return response()->json(['status' => 'success', 'message' => 'You have successfully registered. Activation email has been sent to the user']);
        } catch (\Exception $exception) {
            return response()->json(['status' => 'error', 'message' => $exception->getMessage()], 500);
        }
    }
    ...
    public function mobileLogin(Request $request)
    {
        ...
        try {
            ...
            if ($user){
                ...
                return response()->json(['message' => 'Authenticated', 'code' => 200, 'data' => $user], 200)
                    ->withHeaders([
                        'authorization' => $token,
    ...
    public function login(Request $request)
    {
        ...
        try {
            ...
            if ($token = app('auth')->attempt($credentials)) {
                ...
                return response()->json(['message' => 'Authenticated', 'code' => 200, 'data' => $key], 200)
                    ->withHeaders([
                        'authorization' => $token,
                        'access-control-expose-headers' => 'Cache-Control, Content-Language, Content-Type, Expires, Last-Modified, Pragma, Authorization, Version-Www, Version-Api',
                        'cache-control' => 'no-cache, private'
    ...
    public function email(Request $request)
    {
        ...
        try {
            ...
            if ($user) {
                ...
                Mail::to($user->email)->send(new MailResetPassword(new User(['name' => $user->name, 'email' => $user->email]), $token));
    ...
    public function reset(Request $request)
    {
        ...
        try {
            ...
            if ($reset) {
                ...
                $user = User::query()->where('email', '=', $email)->update(['password' => Hash::make($request->get('password')),'email_verified_at'=>Carbon::now()->toDateTimeString()]);
